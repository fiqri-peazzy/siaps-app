    ...
    <x-slot name="header">
        <div class="flex flex-col gap-y-2">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Manajemen Bobot Prioritas') }}</h2>
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center"><svg class="rtl:rotate-180 w-3 h-3 text-gray-400 mx-1"
                                ...
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="2" d="m1 9 4-4-4-4" />
                            </svg><span class="ms-1 text-sm font-medium text-gray-500 dark:text-gray-400">Bobot
                                Prioritas</span></div>
                    </li>
                </ol>
            </nav>
    ...
    <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
        <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
            <div class="flex items-center mb-4 sm:mb-0">
                <form class="sm:pr-3" action="{{ route('admin.master.priority-bobot.index') }}" method="GET">
                    <div class="relative w-48 mt-1 sm:w-64 xl:w-96">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            ...
                            placeholder="Cari kategori / label...">
                    </div>
                </form>
            </div>
            <div class="flex items-center gap-3">
                <span class="text-sm text-gray-500 dark:text-gray-400">Total: <span
                        class="font-semibold text-gray-900 dark:text-white">{{ $bobots->count() }}</span> data</span>
                <button type="button" data-modal-target="add-priority-modal" data-modal-toggle="add-priority-modal"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="p-8 text-center text-gray-500 dark:text-gray-400">
                                            Data bobot prioritas belum ada.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div id="add-priority-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-md max-h-full transition-transform duration-300 ease-out transform scale-95 opacity-0"
            ...
            <div
                ...
                <form action="{{ route('admin.master.priority-bobot.store') }}" method="POST">
                    @csrf
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Kelompok prioritas, contoh:
                                Lansia, Disabilitas, Ekonomi Lemah</p>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-gray-900 dark:text-gray-200">
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Singkatan/kode unik untuk kategori
                                (tidak boleh sama)</p>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-gray-900 dark:text-gray-200">
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Nama label yang akan ditampilkan
                                di sistem</p>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-gray-900 dark:text-gray-200">
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Semakin tinggi bobot, semakin
                                diprioritaskan (contoh: Lansia 50, Disabilitas 40)</p>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-semibold text-gray-900 dark:text-gray-200">
                                Keterangan (Opsional)
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Penjelasan detail tentang
                                prioritas ini (opsional)</p>
                        </div>
                    </div>
                    <div
                        class="flex items-center p-6 space-x-3 border-t border-gray-200 rounded-b dark:border-gray-700">
                        <button type="submit"
                            class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-bold rounded-xl text-sm px-6 py-3 text-center dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800 transition-all shadow-lg hover:shadow-blue-500/30 shadow-blue-500/20">Simpan
                            Bobot</button>
                ...
                <div
                    ...
                    <form action="{{ route('admin.master.priority-bobot.update', $pb) }}" method="POST">
                        ...
                        <div class="p-6 space-y-4">
                            <div>
                                <label
                                    class="block mb-2 text-sm font-semibold text-gray-900 dark:text-gray-200">Kategori
                            ...
                            <div>
                                ...
                                <input type="text" name="kode" value="{{ $pb->kode }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-all outline-none"
                                    placeholder="Misal: LSK, DSB, EKL, dsb" required>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Singkatan/kode unik (tidak
                                    boleh duplikat)</p>
                            </div>
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Nama label yang ditampilkan
                                </p>
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-gray-900 dark:text-gray-200">Bobot
                                    Prioritas <span class="text-red-500">*</span> (0-100)</label>
                                <input type="number" name="bobot" value="{{ $pb->bobot }}" min="0"
                                    max="100" step="0.01"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-all outline-none"
                                    required>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Semakin tinggi semakin
                                    diprioritaskan</p>
                            </div>
                            <div>
                                <label
                                    class="block mb-2 text-sm font-semibold text-gray-900 dark:text-gray-200">Keterangan
                                    (Opsional)
                                </label>
                                <textarea name="keterangan" rows="3"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-all outline-none"
                                    placeholder="Penjelasan tambahan tentang kategori prioritas ini...">{{ $pb->keterangan }}</textarea>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Penjelasan detail (opsional)
                                </p>
                            </div>
                            <div>
                                ...
                                <select name="is_active"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-all outline-none">
                                    <option value="1" {{ $pb->is_active ? 'selected' : '' }}>✓ Aktif (Digunakan)
                                    </option>
                                    <option value="0" {{ !$pb->is_active ? 'selected' : '' }}>✗ Nonaktif (Tidak
                                        Digunakan)</option>
                                </select>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Aktif = akan digunakan dalam
                                    perhitungan prioritas</p>
                            </div>
                        </div>
                        <div
                            class="flex items-center p-6 space-x-3 border-t border-gray-200 rounded-b dark:border-gray-700">
                            <button type="submit"
                                class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-bold rounded-xl text-sm px-6 py-3 text-center dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800 transition-all shadow-lg hover:shadow-blue-500/30 shadow-blue-500/20">Update
                                Bobot</button>
